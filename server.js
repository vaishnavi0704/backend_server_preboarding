...
//                   silence_duration_ms: 500,
//                   create_response: true,
//                   interrupt_response: true // 🔥 KEY: Enables interruption when user speaks
//                 },
//                 temperature: 0.8,
...
//             // 🔥 NEW: Log when user starts speaking (interruption detected)
//             if (event.type === 'input_audio_buffer.speech_started') {
//               console.log('🎤 User started speaking - voice agent will be interrupted');
//               ws.send(JSON.stringify({
//                 type: 'user_speaking',
//                 status: 'started'
//               }));
...
//             // 🔥 NEW: Log when user stops speaking
...
//             // 🔥 NEW: Log when response is cancelled due to interruption
//             if (event.type === 'response.cancelled') {
//               console.log('🛑 Response cancelled - user interrupted');
//             }

//             // Only process audio when not handling verification
//             if (event.type === 'response.audio.delta' && !isProcessingVerification) {
//               ws.send(JSON.stringify({
//                 type: 'audio_delta',
//                 delta: event.delta
...
//             // 🔥 NEW: Add more transcription debug events
...
//               ws.send(JSON.stringify({ 
//                 type: 'agent_message', 
//                 content: 'Switching to standard mode. Please continue.' 
//               }));
//             }
//           });
...
//         // 🔥 NEW: Frontend confirms audio is ready to receive again
//         console.log('🔊 Frontend audio ready - user can speak now');
//         isProcessingVerification = false;
        
//         const realtimeWs = realtimeConnections.get(sessionId || data.recordId);
//         if (realtimeWs?.readyState === WebSocket.OPEN) {
//           console.log('✅ Voice agent ready for user input');
//         }

//       } else if (data.type === 'audio_ready') {
//         // 🔥 Frontend confirms audio is ready to receive again
//         console.log('🔊 Frontend audio ready - resuming voice agent');
//         isProcessingVerification = false;
        
//         const realtimeWs = realtimeConnections.get(sessionId || data.recordId);
//         if (realtimeWs?.readyState === WebSocket.OPEN) {
//           console.log('✅ Voice agent unlocked - user can speak now');
//         }
...
//             if (nextStep === 'complete') {
//               ws.send(JSON.stringify({ type: 'complete' }));
//               console.log('\n🎉 ALL DOCUMENTS COMPLETE - FINAL TRANSCRIPT:');
//               printConversationLog(sessionId);
//               printDocumentSummary(sessionId);
//             } else {
//               // 🔥 NEW: Print conversation log after each document step
//               console.log(`\n📋 TRANSCRIPT AFTER ${session.currentStep.toUpperCase()} DOCUMENT:`);
//               printIntermediateConversationLog(sessionId);
//             }
//           }
